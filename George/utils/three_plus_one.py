...
valid_dates = get_valid_dates(conn, date_from=20190101, date_until=20210324)
...
def three_plus_one_bin_slice(daily_data, date_key_int: int, slice_size: int) -> bool:
    idx = daily_data[daily_data['date_key_int'] == date_key_int].index.tolist()[0]
...
def validate_three_plus_one_op(conn, sec_symbol: str,
    ...
    daily_data = download_data_daily(conn,
                                     ...
                                     end_date=valid_dates[ed_idx + slice_size],
                                     interval="daily")
...
secs = read_sec_list('securities_sg4.csv')
...
for each in secs:
    try:
        ...
        print(f" mean = {df.mean()}, std = {df.std()}")
    except Exception as e:
        pass
